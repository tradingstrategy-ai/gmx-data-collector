...
# Public HTTPS fallback when the script is run outside a git working tree.
_DEFAULT_REMOTE_URL = "https://github.com/tradingstrategy-ai/gmx-data-collector.git"


def _resolve_origin_url() -> str:
    """Return the git origin URL of the repo containing this script.

    Prefers the user's configured ``origin`` remote so SSH vs HTTPS matches
    their existing clone. Falls back to the public HTTPS URL when the
    script is executed outside a git working tree.
    ...
    try:
        ...
        return result.stdout.strip() or _DEFAULT_REMOTE_URL
    except (subprocess.CalledProcessError, FileNotFoundError):
        return _DEFAULT_REMOTE_URL


def _seed_from_branch(output_dir: Path, ref: str) -> dict:
    """Seed ``output_dir`` from the remote data branch.

    Shallow-clones ``ref`` into a temp directory, then merge-copies
    ``user_data/`` into the local ``output_dir``. Existing local files are
    never overwritten — only missing files are copied. This keeps the
    operation reversible (user can ``rm -rf user_data/`` to undo) and
    protects any in-flight local edits.

    :param output_dir: Local destination root (e.g. ``./user_data``).
    :param ref: Remote branch to seed from (e.g. ``data/daily-collection``).
    :return: Summary dict with ``copied``, ``skipped``, ``bytes`` keys.
        On failure returns a dict with ``error`` set instead.
    """
    url = _resolve_origin_url()
    ...
    try:
        ...
        if not src_root.is_dir():
            console.print(
                f"  [yellow]Warning:[/yellow] branch has no user_data/ directory"
            )
...
def _print_coverage_summary(output_dir: Path) -> None:
...
Examples:
  ...
  poetry run python scripts/collect_daily_snapshot.py -q --seed-only
    ...
    parser.add_argument(
        "--quickstart-ref",
        default="data/daily-collection",
        help="Remote branch to seed from (default: data/daily-collection).",
    )
    ...
    if args.quickstart:
        ...
        summary = _seed_from_branch(args.output_dir, args.quickstart_ref)
        if "error" not in summary:
            ...
            _print_coverage_summary(args.output_dir)
        else:
            console.print(
