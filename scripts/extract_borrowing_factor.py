...
# ///
"""
GMX V2 Borrowing Factor Extractor (HyperSync Edition)
======================================================
Extracts ``Borrowing`` events from GMX V2 EventEmitter to get the market-level
``borrowingFactorPerSecond`` — the actual per-second borrowing rate as a
30-decimal fixed-point integer.

The borrowing rate is charged to the dominant side (larger OI) and goes to LPs.
This is the on-chain equivalent of the complex Method 1/Method 2 calculation
in the Dune Analytics query (``dune."gmx-io".result_borrowing_rate_all_data``).

Outputs:
- Raw events: ``data/borrowing/{network}/raw/borrowing/{SYMBOL}/partition=0/data.parquet``
- Aggregated rates: ``data/borrowing/{network}/rates/{SYMBOL}/1h.parquet``

The aggregated ``rates/`` files can be combined with funding rate files from
``extract_funding_factor.py`` to compute net rates (funding + borrowing).
...
USAGE
-----
    poetry run python scripts/extract_borrowing_factor.py [OPTIONS]
...
-------
    ...
    --output-dir     Base output directory (default: ./data/borrowing)
...
--------
    ...
    poetry run python scripts/extract_borrowing_factor.py --from-block 290000000 --to-block 290100000 --output json
...
# Precision: borrowingFactorPerSecond is a 30-decimal fixed-point integer
BORROWING_FACTOR_PRECISION = 10**30
...
# Borrowing event hash (topic1)
BORROWING_EVENT_HASH = "0x" + keccak(text="Borrowing").hex()
...
# GMX V2 Market Addresses — copied from extract_funding_factor.py
...
MARKETS = {
    ...
    # Swap-only markets (may emit Borrowing events with 0 rate)
...
@dataclass
class BorrowingFactorRecord:
    """Raw Borrowing event from GMX V2 EventEmitter.

    The ``Borrowing`` event carries ``borrowingFactorPerSecond`` — the per-second
    borrowing rate as a 30-decimal fixed-point integer. This is the rate charged
    to the dominant side (larger OI) and goes to LPs (not other traders).

    :ivar symbol: Derived symbol (e.g., ``'ETH'``)
    :ivar market: Market contract address
    :ivar borrowing_factor_per_second: Raw 30-decimal integer as string
    :ivar borrowing_rate_per_second: Decimal per-second rate
    ...
    symbol: str
    market: str
    borrowing_factor_per_second: str
    borrowing_rate_per_second: float
    block_number: int
    block_timestamp: int
    block_datetime: str
...
async def extract_borrowing_events(
    ...
    The ``Borrowing`` event carries ``borrowingFactorPerSecond`` — the
    pre-computed per-second borrowing rate. This is the on-chain equivalent
    of the complex Method 1/Method 2 calculation in the Dune query.
    ...
    :returns: List of :class:`BorrowingFactorRecord` objects.
    """
    ...
    with Progress(
        ...
        async for response in _stream_with_retry(client, query):
            ...
            for log in response.data.logs:
                ...
                # Skip swap-only markets (no perpetual trading)
                market_info = MARKETS.get(market_addr.lower())
                if market_info and market_info.get("indexToken") is None:
                    continue

                factor_raw = decoded["uints"].get("borrowingFactorPerSecond", 0)
                ...
                record = BorrowingFactorRecord(
                    symbol=symbol,
                    market=market_addr.lower(),
                    borrowing_factor_per_second=str(factor_raw),
                    borrowing_rate_per_second=rate,
    ...
    console.print(
        ...
        f"{len(records):,} Borrowing events from {total_logs:,} logs"
    )
    if decode_errors:
...
def aggregate_hourly_rates(
    ...
    """Aggregate raw Borrowing events into hourly rate snapshots per symbol.

    Produces one DataFrame per symbol with columns matching the output schema.

    :param records: List of :class:`BorrowingFactorRecord` objects.
    ...
    hourly = (
        ...
        .agg(
            pl.col("borrowing_rate_per_second").mean().alias("borrowing_rate"),
            pl.col("borrowing_rate_per_second").min().alias("borrowing_rate_min"),
            pl.col("borrowing_rate_per_second").max().alias("borrowing_rate_max"),
    ...
    hourly = hourly.with_columns(
        (pl.col("borrowing_rate") * 3600).alias("borrowing_rate_hourly"),
        (pl.col("borrowing_rate") * 3600 * 8760).alias("borrowing_rate_annualized"),
    )
...
def save_raw_per_symbol(records: list[BorrowingFactorRecord], output_dir: Path) -> None:
    """Save raw Borrowing events to per-symbol Parquet files.

    :param records: List of :class:`BorrowingFactorRecord` objects.
    :param output_dir: Base output directory (e.g., ``data/borrowing/arbitrum``).
    """
    if not HAS_POLARS:
        console.print("[red]polars required for Parquet[/red]")
        return

    by_symbol: dict[str, list[BorrowingFactorRecord]] = defaultdict(list)
    for r in records:
        by_symbol[r.symbol].append(r)

    for symbol, sym_records in sorted(by_symbol.items()):
        filepath = output_dir / "raw" / "borrowing" / symbol / "partition=0" / "data.parquet"
...
def print_summary(records: list[BorrowingFactorRecord]) -> None:
    ...
    :param records: List of :class:`BorrowingFactorRecord` objects.
    """
    table = Table(title="Borrowing Factor Summary", show_lines=False)
    ...
    for symbol in sorted(by_symbol.keys()):
        ...
        avg_rate = sum(r.borrowing_rate_per_second for r in sym_records) / len(sym_records)
...
async def async_main(args: argparse.Namespace) -> None:
    ...
    checkpoint_path = checkpoint_dir / "borrowing_factor_checkpoint.json"
    ...
    console.print(
        Panel(
            "\n".join(header_lines),
            title="GMX V2 Borrowing Factor Extractor",
    ...
    console.print()
    records = await extract_borrowing_events(
        client=client,
    ...
    if not records:
        console.print("\n[yellow]No Borrowing events found.[/yellow]")
        if args.resume:
            save_checkpoint(
                checkpoint_path,
                last_block=to_block,
    ...
    if args.output == "parquet":
        ...
        base = f"gmx_v2_borrowing_factor_{args.network}_{from_block}_{to_block}_{ts}"
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Extract GMX V2 borrowing factor data using HyperSync",
...
Examples:
  ...
  poetry run python scripts/extract_borrowing_factor.py --resume --background
    ...
    parser.add_argument(
        "--output-dir",
        type=str,
        default="./data/borrowing",
        help="Base output directory (default: ./data/borrowing)",
    )
    parser.add_argument(
        "--output",
    ...
    parser.add_argument(
        "--log-file",
        type=str,
        default="./logs/borrowing_factor.log",
        help="Log file for background mode",
    )
    parser.add_argument(
        "--pid-file",
        type=str,
        default="./logs/borrowing_factor.pid",
